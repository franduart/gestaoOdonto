    ...
    <div className="flex flex-col gap-3">
      <Card>
       <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
        ...
        <DialogContent className="sm:max-w-[245px]">
          <DialogHeader>
       ...
       <CardContent>
        {reminder.length === 0 && (
          ...
          >Nenhum lembrete registrado...</p>
        )}
        <ScrollArea className="h-[340px] lg-max-h-[calc(100vh-15rem)] w-full pr-0 flex-1">
         {reminder.map((item)=>(
            <article 
            key={item.id} 
            className="flex flex-wrap flex-row items-center 
            justify-between py-2 bg-amber-200 mb-2 px-2 rounded-md">
